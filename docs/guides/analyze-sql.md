...
supported SQL facts. Resolved SQL edges do not become Python-like `callers`,
`impact`, `unreferenced`, `surface`, `consumers`, or `system-deps` edges; the
existing generic unresolved-reference recall remains available.
