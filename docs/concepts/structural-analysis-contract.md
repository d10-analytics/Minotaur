...
statement. Direct imports in an `if` suite use the same source-position
producer and participate in the bounded structural join described below.
...
The AC-01 through AC-05 rows below are natural public proofs. “Owner” is the
...
| AC-03 | Deferred module bodies use final module state; nested deferred functions, lambdas, and methods use final enclosing-function state. Defaults and parameters retain immediate or lexical behavior, callable-local imports do not mutate siblings, and direct `if` arms join their structural post-condition state. Within a compound statement, an incoming route remains available to ordered reads until a direct write; affected roots are uncertain at continuation. A `try` success path supplies its `else` state, handler types read before their `as` targets, and match-guard writes affect later guards and continuation. | `test_module_default_before_import_and_deferred_body_use_final_state`, `test_nested_callables_use_final_enclosing_function_import_state`, `test_module_lambda_body_uses_final_state_with_immediate_defaults_and_parameters`, `test_nested_callable_import_does_not_mutate_enclosing_or_sibling_routes`, `test_conditional_named_alias_and_relative_imports_join_exact_routes`, `test_conditional_divergent_and_omitted_routes_are_one_unresolved_focal_use`, `test_conditional_plain_alias_root_disagreement_blocks_call_and_load`, `test_conditional_uncertain_imports_do_not_resolve_outer_declarations`, `test_conditional_all_terminated_arms_retain_agreeing_route`, `test_nested_dotted_import_reestablishes_existing_direct_prefix`, `test_compound_body_preserves_incoming_call_and_reference_before_mutation`, `test_try_handler_starts_after_possible_try_body_write_but_before_handler_write`, `test_try_else_preserves_success_body_state_and_post_try_uncertainty`, `test_try_handler_type_reads_import_before_same_named_exception_target`, `test_try_body_write_still_blocks_same_named_handler_type`, and `test_match_guard_walrus_blocks_later_guards_and_continuation` |
...
| Module binders invalidate every affected prefix while lexical descriptors retain nearest routes. | A module binder leaves an affected module use unresolved; a direct function-local route resolves in that function, while class-only and non-`if` control-flow nested plain imports remain syntactic. A class import may refine an already-visible plain package route. | `test_dotted_import_module_binders_invalidate_every_prefix_use`, `test_dotted_import_module_binders_include_legal_definition_headers`, `test_dotted_import_lexical_descriptors_preserve_nearest_routes`, and `test_nested_plain_dotted_imports_are_syntactic_only_in_each_container` |
...
| AC-04 | Direct function-body imports and supported direct `if` suites resolve calls and loads through the shared owner and resolver. Each `if` arm is isolated; class global/nonlocal effects remain in their enclosing overlay, class locals stay hidden from methods, deferred callables use final owner state, and an inner precise `if` does not make an enclosing loop continuation precise. Imports in loop, `try`, `with`, and `match` suites retain their conservative continuation behavior. | `test_function_local_import_routes_bind_calls_and_loads_at_source_positions`, `test_lexical_class_route_installation_deletion_and_method_restoration`, `test_nested_plain_dotted_imports_remain_syntactic_only`, `test_nested_plain_dotted_imports_are_syntactic_only_in_each_container`, `test_conditional_class_directives_invalidate_owner_without_sibling_leak`, `test_conditional_class_nonlocal_write_preserves_sibling_module_route`, `test_conditional_deferred_callables_use_reimported_owner_and_skip_class_locals`, and `test_inner_if_in_loop_resolves_prewrite_use_but_freezes_post_loop_route` |
| AC-05 query meaning | A resolved conditional call produces the same `CALLS` edge, caller result, and inbound impact as any other resolved call. A resolved conditional load produces a `REFERENCES` edge and counts as a use without creating a caller or inbound impact. A divergent conditional route remains unresolved and is recall-only for callers; it does not create a resolved impact path or hide the target from `unreferenced`. | `test_conditional_call_graph_drives_exact_callers_with_location` and `test_ambiguous_conditional_continuation_is_callers_recall_only` in `tests/query/test_symbols.py`; `test_conditional_call_graph_drives_inbound_impact`, `test_ambiguous_conditional_continuation_has_no_inbound_impact_path`, and `test_conditional_load_graph_does_not_create_inbound_impact` in `tests/query/test_impact.py`; and `test_conditional_load_keeps_target_out_of_unreferenced` and `test_ambiguous_conditional_continuation_stays_unreferenced` in `tests/query/test_unreferenced.py` |
...
| Both direct `if` arms bind the same target. | **Supported structurally.** Each explicit arm contributes an isolated post-condition state; an omitted `else` contributes the incoming state. Canonical root and route-category agreement preserves the common route, while each `IMPORTS` relationship remains separate evidence. This is source-structure composition, not runtime reachability. |
| Direct `if` arms bind different roots, categories, or qualified prefixes. | **Supported conservatively.** The differing or missing root/prefix becomes uncertain, descendants cannot use a stale route, and a direct reimport can re-establish only the route it actually writes. A conditional-only module-root replacement therefore remains unresolved rather than falling through to an outer declaration. |
| A direct `if` arm returns or raises, or an omitted `else` leaves the name unchanged. | **Supported structurally.** Return and raise arms still contribute binding data; frozen tails cannot invent a route, and a use after the join remains unresolved when a continuing arm lacks the binding. When all arms terminate after establishing the same route, a structural tail retains that route. No reachability or invocation claim is made. |
| A call or load inside a direct importing `if` arm can resolve to that arm's target. | **Supported at the source position.** Direct named, aliased, relative-named, and plain dotted imports in the current `if` suite use the shared resolver for contained calls and loads. Nested `if` composition is bounded; an enclosing loop, `try`, `with`, or `match` keeps its conservative continuation policy. |
...
| A loop, `try`, `with`, or `match` suite may establish a route for its continuation. | **Outside this slice.** Imports in these non-`if` compound suites remain syntactic evidence. A direct `if` nested inside one can resolve an ordered pre-write use, but the enclosing compound's continuation remains conservative and possibly changed roots become uncertain. |
...
These limits are deliberate. The current slice does not add general
control-flow joins beyond the supported direct `if` composition, control-flow-
local import production in non-`if` compounds, invocation timing,
interprocedural side effects, a second resolver, a declaration-owner or
declaration-kind precedence rule, runtime dispatch, graph-schema or
persistent-state changes, or activation of unrelated dormant flow machinery.
It makes no JavaScript-specific claim.
